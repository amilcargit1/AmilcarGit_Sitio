...
if (typeof firebase !== 'undefined') {
  ...
  auth.onAuthStateChanged((user) => {
    ...
    if (user) {
      ...
      notifUnsub = db.collection('notifications').where('toUid', '==', user.uid)
        ...
        .onSnapshot((snap) => {
          ...
          if (notifList) {
            notifList.innerHTML = '';
            if (snap.empty) {
              notifList.innerHTML = '<li class="notif-empty">Sin notificaciones todavía</li>';
            }
            snap.forEach((nDoc) => {
              const n = nDoc.data();
              ...
              const text = n.type === 'follow'
                ? `${escapeHtml(n.fromName || 'Alguien')} empezó a seguirte`
                : `${escapeHtml(n.fromName || 'Alguien')} comentó en "${escapeHtml(n.songTitle || 'tu canción')}"`;
              li.textContent = text;
              li.addEventListener('click', () => {
                db.collection('notifications').doc(nDoc.id).update({ read: true }).catch(() => {});
                if (n.type === 'follow' && n.fromUid) showProfile(n.fromUid);
              });
  ...
  document.addEventListener('click', (e) => {
    if (notifPanel && !notifPanel.hidden && !e.target.closest('.notif-wrap')) {
  ...
  function hashColor(str) {
    ...
    const palettes = [
      ['#2E6BFF', '#0F1B2D'], ['#FF3B3B', '#0F1B2D'], ['#C9A227', '#0F1B2D'],
      ['#2E6BFF', '#FF3B3B'], ['#C9A227', '#2E6BFF'],
    ];
  ...
  function renderSongItem(doc, container) {
    ...
    const [c1, c2] = hashColor(song.title || doc.id);
    const initial = (song.title || '♪').trim().charAt(0).toUpperCase();
    li.innerHTML = `
      <div class="track-row">
        <div class="track-art" style="background: linear-gradient(135deg, ${c1}, ${c2});">
          <span class="track-art-letter">${escapeHtml(initial)}</span>
          <button type="button" class="track-play" data-song-id="${doc.id}" aria-label="Reproducir">▶</button>
        </div>
        <div class="track-info">
          <div class="track-head">
            <span class="track-title">${escapeHtml(song.title || 'Sin título')}</span>
            ${(isOwner || isAdmin) ? `<button type="button" class="comunidad-delete" data-id="${doc.id}" data-uid="${song.uid}">✕</button>` : ''}
          </div>
          <span class="track-by" data-uid="${escapeAttr(song.uid)}">${escapeHtml(song.name || 'Anónimo')}</span>
          <div class="track-progress">
            <span class="track-time track-time-current">0:00</span>
            <input type="range" class="track-seek" min="0" max="100" value="0" data-song-id="${doc.id}">
            <span class="track-time track-time-total">0:00</span>
          </div>
        </div>
      </div>
      <audio class="track-audio" src="${escapeAttr(song.url || '')}" preload="metadata"></audio>
    ...
    const audioEl = li.querySelector('.track-audio');
    const playBtn = li.querySelector('.track-play');
    const seekBar = li.querySelector('.track-seek');
    const timeCurrentEl = li.querySelector('.track-time-current');
    const timeTotalEl = li.querySelector('.track-time-total');

    playBtn.addEventListener('click', () => {
      // pausa cualquier otra canción que esté sonando
      document.querySelectorAll('.track-audio').forEach((a) => {
        if (a !== audioEl && !a.paused) {
          a.pause();
          const otherBtn = a.closest('.track-row')?.querySelector('.track-play');
    ...
    li.querySelector('.track-by').addEventListener('click', () => {
    ...
    if (commentForm) {
      commentForm.addEventListener('submit', async (e) => {
        ...
        // Notificación al dueño de la canción (si no es uno mismo comentando)
        if (song.uid && song.uid !== currentUser.uid) {
          db.collection('notifications').add({
            toUid: song.uid,
            fromUid: currentUser.uid,
            fromName: myNickname || currentUser.displayName || 'Anónimo',
            type: 'comment',
            songId: doc.id,
